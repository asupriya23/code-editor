...
import { languageMap, PlaygroundContext } from '../../context/PlaygroundProvider'
...
const Playground = () => {
  ...
  const getOutput = async (token) => {
    const options = {
        ...
        params: { base64_encoded: 'true', fields: '*' },
        headers: {
            'X-RapidAPI-Key': '0b87839e9emsh1813b1fb993acb7p1a6fa5jsn2d2c6e95db2f',
            'X-RapidAPI-Host': 'judge0-ce.p.rapidapi.com'
        }
    };
    ...
    for (let i = 0; i < 10; i++) { // Retry up to 10 times
        const res = await axios.request(options);
        if (res.data.status_id > 2) return res.data;
  ...
  const runCode = async () => {
    ...
    // console.log("Decoded Response Data:", res.data);
